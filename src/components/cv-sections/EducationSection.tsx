...
export default function EducationSection() {
  ...
  // Trigger auto-save when form data or courses change
  useEffect(() => {
    if (editingItemId) {
      triggerAutoSave();
    }
  }, [formData, courses, editingItemId]);
    ...
    <div className="p-2.5 md:p-4 bg-gray-50 rounded-lg border border-gray-100">
      <div className="flex justify-between items-center mb-2 md:mb-3">
        <div className="flex items-center gap-2">
          <h3 className="text-xs md:text-sm font-medium">{isEdit ? "Edit" : "Add"} Education</h3>
          {isEdit && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 md:gap-4">
        ...
        <div className="md:col-span-2">
          ...
          <div className="flex gap-2">
            ...
            />
            <Button size="sm" onClick={addCourse} disabled={!newCourse.trim()} className="h-8 bg-black hover:bg-black/80 text-xs px-3 !text-white">Add</Button>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-2 md:mb-4">
        <h2 className="text-base md:text-lg font-medium">Education</h2>
